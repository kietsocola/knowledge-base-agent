...
export function ChatInterface({
  ...
  // Track message count and watch for evaluation trigger
  useEffect(() => {
    const userMsgCount = messages.filter((m) => m.role === "user").length
    setMessageCount(userMsgCount)

    // Check last assistant message for evaluation trigger
    const lastMsg = messages.at(-1)
    if (lastMsg?.role === "assistant") {
      const meta = lastMsg.metadata as
        | { triggerEvaluation?: boolean }
        | undefined
      if (meta?.triggerEvaluation) {
        ...
        <div className="px-4 pb-4 pt-2 border-t border-border shrink-0">
          ...
          <div className="flex items-center justify-between mt-1.5">
            ...
            <button
              type="button"
              onClick={() => {
                const lastUserPart = messages.filter(m => m.role === "user").at(-1)?.parts.find((p: { type: string }) => p.type === "text") as { type: "text"; text: string } | undefined
