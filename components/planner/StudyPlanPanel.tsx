  ...
  return (
    <div className="paper-surface rounded-[2rem] p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="section-label">Planner flow</div>
        ...
        <div className="rounded-full border border-border/60 bg-card/80 px-4 py-2 text-xs font-bold text-primary">
          {plan.horizonLabel}
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        {plan.steps.map((step, index) => (
          <div key={step.id} className="metric-tile rounded-[1.5rem] p-5">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[0.58fr_0.42fr]">
        <div className="metric-tile rounded-[1.5rem] p-5">
          ...
          <div className="mt-4 space-y-3">
            {plan.suggestedPrompts.map((prompt, index) => (
              <div key={index} className="rounded-[1.2rem] border border-border/50 bg-card/85 px-4 py-3 text-sm shadow-sm">
                {prompt}
              </div>
            ))}
          </div>
        ...
        <div className="ink-panel rounded-[1.5rem] p-5">
          <div className="flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-secondary" />
            <div className="text-sm font-bold">Tín hiệu thành công</div>
          </div>
          <div className="mt-4 rounded-[1.2rem] border border-white/10 bg-white/8 px-4 py-4 text-sm leading-relaxed shadow-sm">
            {plan.successSignal}
          </div>
