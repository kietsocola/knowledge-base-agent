...
const agentTone = {
  "Diagnosis Agent": "bg-amber-100 text-amber-700",
  "Planner Agent": "bg-cyan-100 text-cyan-700",
  "Tutor Agent": "bg-emerald-100 text-emerald-700",
} as const
    ...
    <div className="rounded-[2rem] border border-white/70 bg-white/85 p-6 shadow-sm">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        {plan.steps.map((step, index) => (
          <div key={step.id} className="rounded-[1.5rem] border border-white/80 bg-slate-50/80 p-5">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[0.58fr_0.42fr]">
        <div className="rounded-[1.5rem] border border-white/80 bg-slate-50/80 p-5">
          ...
          <div className="mt-4 space-y-3">
            {plan.suggestedPrompts.map((prompt, index) => (
              <div key={index} className="rounded-[1.2rem] bg-white px-4 py-3 text-sm shadow-sm">
                {prompt}
              </div>
            ))}
          </div>
        ...
        <div className="rounded-[1.5rem] border border-white/80 bg-slate-50/80 p-5">
          ...
          <div className="mt-4 rounded-[1.2rem] bg-white px-4 py-4 text-sm leading-relaxed shadow-sm">
            {plan.successSignal}
          </div>
