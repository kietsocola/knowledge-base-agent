...
test("buildClassroomOverview summarizes classroom activity and student risk", () => {
  ...
  assert.equal(overview.activityTimeline[0]?.chatTurns, 3)
  assert.equal(overview.activityTimeline[0]?.evaluations, 1)
  assert.equal(overview.interventionAlerts.length, 2)
  assert.equal(overview.interventionAlerts[0]?.id, "class-high-attention-ratio")
...
test("buildClassroomOverview handles empty classroom data", () => {
  ...
  assert.deepEqual(overview.activityTimeline, [])
  assert.equal(overview.interventionAlerts.length, 1)
  assert.equal(overview.interventionAlerts[0]?.id, "class-stable")
